 ...
 * Ensures that the difference between sunrise and iqamah is at least 15 minutes.
...
export function calculateFajrIqamahFromSunrise(sunriseTime) {
  ...
  // Round down to the nearest 15-minute block before sunrise
  let iqamahMinutes = Math.floor(totalMinutes / 15) * 15;

  if (totalMinutes - iqamahMinutes < 15) {
    iqamahMinutes -= 15;
  }
...
export function calculateIshaIqamahTime(adhanTime24) {
  ...
  // +5 minute buffer
  let totalMinutes = hour * 60 + minute + 3;
